...
static __constant__ uint8_t d_sub_byte[16][16];
...
__device__ __forceinline__ void cn_aes_set_key( uint8_t *key, const uint8_t * data )
{
    int i, j;
	size_t x, y, data_len = AES_KEY_SIZE; // 32
    int key_base = data_len / AES_RKEY_LEN; // 32 / 4 = 8
    int num_keys = key_base + AES_ROUND_BASE; // 8 + 7 = 15
    uint8_t temp[AES_COL_LEN];
    uint8_t tempkey[240];
    const uint8_t aes_gf_8[] = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36 };
    
    MEMSET8(tempkey, 0, 30);
    MEMCPY8(tempkey, data, 4);

	for( i = key_base; i < num_keys * AES_RKEY_LEN; i++ ) { // i = 8 ; i < 15 * 4 = 60 -> 52 rounds

		*(uint32_t *)temp = *(uint32_t *)&tempkey[(i - 1) * AES_RKEY_LEN];
		
		if( i % key_base == 0 ) { // i IN (8, 16, 24, 32, 40, 48 )

            ROTWL(temp);

			for( j = 0; j < AES_COL_LEN; j++ ) {

                y = ((x = *(temp + j)) >> 4) & 0x0f;
                x &= 0x0f;
                *(temp + j) = d_sub_byte[y][x];
            }

			temp[0] = temp[0] ^ aes_gf_8[i / key_base - 1];
		}
		else if( key_base > 6 && i % key_base == 4 ) { // i IN ( 12, 20, 28, 36, 46 )
			for( j = 0; j < AES_COL_LEN; j++ ) {
		...
		for( j = 0; j < AES_COL_LEN; j++ ) {
            ...
            tempkey[i * AES_RKEY_LEN + j] =
				tempkey[(i - key_base) * AES_RKEY_LEN + j] ^ temp[j];
		}
	}

    MEMCPY8(key, tempkey, 20);
}
